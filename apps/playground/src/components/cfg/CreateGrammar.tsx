...
const CreateGrammarContext = React.createContext({} as ReturnType<typeof useGrammarInput>)

function GrammarInput () {
  const {userInputGrammar, updateRuleVariable, removeRule, addRule, updateToken, addSubstitution, addToken, removeToken, removeSubstitution} = useContext(CreateGrammarContext)
  return <FlexCol>
...
export function CreateGrammar() {
  ...
  const grammarInputHook = useGrammarInput();
  const { userInputGrammar, productionRules, resetState } = grammarInputHook;
  ...
  return <CreateGrammarContext.Provider value={grammarInputHook}>
    <Flex sx={{
      p: 2,
      height: "100%"
    }}>
      <FlexCol sx={{
        ...
        <TextField placeholder="Grammar label" value={grammarLabel} onChange={(e) => setGrammarLabel(e.target.value)} />
        <GrammarInput />
